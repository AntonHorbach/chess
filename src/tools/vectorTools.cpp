...
namespace tools {
            ...
            const size_t MAX)
    {
        ...
        for(size_t i = 0; i < source.size(); ++i)
        {
            ...
            {
                if(x + source[i].first == filter[j].first
                   && y + source[i].second == filter[j].second)
                {
                    Memory mem;
        ...
        for(int i = 0; i < res.size(); ++i)
        {
            ...
            for(size_t j = 0; j < memory.size(); ++j)
            {
                ...
                int count =
                        std::max(abs(memory[j].x - res[i].first), abs(memory[j].y - res[i].second));
                ...
                for(size_t k = 1; k <= count + 1; ++k)
                {
                    ...
                    {
                        res.erase(std::remove(std::begin(res), std::end(res), res[i]),
                                  std::end(res));
                        i = -1;
                        break;
                    }
            ...
            const std::vector<std::pair<int, int>>& filter)
    {
        ...
        for(size_t i = 0; i < source.size(); ++i)
        {
            if(std::find(std::begin(filter),
                         std::end(filter),
                         source[i] + std::pair<int, int>{x, y}) != std::end(filter))
            {
                res.push_back(source[i]);
            }
        }
    ...
    std::vector<std::pair<int, int>> findMatches(
            int x1, int y1, int x2, int y2, const std::vector<std::pair<int, int>>& source)
    {

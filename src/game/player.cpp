...
Player::Player(Color _color): color(_color), figures(16)
{
...
bool Player::init(SDL_Renderer* renderer)
{
    ...
    for(size_t i = 0; i < figures.size(); ++i)
    {
        if(figures[i].getType() == FT::LEFT_ROOK || figures[i].getType() == FT::RIGHT_ROOK)
        {
            actions = tools::generateLongStraightMoves();
            figures[i].setAttacks(actions);
            figures[i].setMoves(actions);
        }
        else if(figures[i].getType() == FT::LEFT_HORSE || figures[i].getType() == FT::RIGHT_HORSE)
        {
            actions = tools::generateMoves({{2, 1},
                                            {1, 2}});
        ...
        {
            actions = tools::generateMoves({{1, 1},
                                            {0, 1},
                                            {1, 0}});
...
void Player::update(const Player* enemy)
{
    ...
    for(size_t i = 0; i < figures.size(); ++i)
    {
        ...
        available_moves =
                tools::clipping(figures[i].getX(), figures[i].getY(), figures[i].getMoves(), poses);

        available_attacks = tools::findMatches(figures[i].getX(),
                                               figures[i].getY(),
                                               tools::clipping(figures[i].getX(),
                                                               figures[i].getY(),
                                                               figures[i].getAttacks(),
                                                               poses,
                                                               false),
                                               enemy_poses);
        ...
        if(i == ind_FT::KING)
        {
            ...
            for(const Figure& figure : enemy->getFigures())
            {
                ...
                {
                    delta = figure.getAvailableMoves()
                            + figure.getAvailableAttacks()
                ...
                coveredFigures += tools::findMatches(figure.getX(),
                                                     figure.getY(),
                                                     tools::clipping(figure.getX(),
                                                                     figure.getY(),
                                                                     figure.getAttacks(),
                                                                     enemy_poses,
                                                                     false),
            ...
            if(color == WHITE)
            {
                if(figures[i].getY() != 6)
                {
                    available_moves.erase(std::remove(std::begin(available_moves),
                                                      ...
                                                      MOVE{0, -2}), std::end(available_moves));
                }
            }
            else
            {
